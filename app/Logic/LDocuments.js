...
module.exports = {
    ...
    publishFoundDocument: function (req, data, callb) {
        user = lSession.getUserFromSession(req);
            ...
            if (doc) {//Ya se habia denunciado
                if (doc.found) {
                    dDocuments.insertDocument(
                        ...
                        [data.type, data.number, doc.publication_date, 'true', Date.now(), data.finder, data.description],
                        function (err, result) {
                            callb(err, { 'exists': true, 'found': true, 'data': undefined })
                        });
                } else {
                    dDocuments.insertDocument(
                        ['type', 'number', 'publication_date', 'recovered', 'recovery_date', 'finder', 'description'],
                        [data.type, data.number, doc.publication_date, 'true', Date.now(), data.finder, data.description],
                        function (err, result) {
                            callb(err, { 'exists': true, 'found': false, 'data': result })
                        });
                }
            } else {
                dDocuments.insertDocument(
                    ['type', 'number', 'finder', 'description', 'publication_date', 'recovered'],
                    [data.type, data.number, data.finder, data.description, Date.now(), false],
                    function (err, result) {
                        callb(err, { 'exists': false, 'found': false, 'data': undefined })
                    });
            }
        })
    },
